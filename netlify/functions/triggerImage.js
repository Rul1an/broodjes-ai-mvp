...
exports.handler = async function (event, context) {
    console.log('--- triggerImage handler started ---'); // Renamed log message
    ...
    if (!ingredient_id || typeof ingredient_id !== 'number' || !Number.isInteger(ingredient_id) || ingredient_id <= 0) {
        console.error('Validation failed for ingredient_id:', ingredient_id);
        return {
            statusCode: 400,
    ...
    if (!supabase) {
        console.error('triggerImage: Failed to initialize Supabase service client.');
    ...
    try {
        ...
        console.log(`Successfully initiated GCF call for ingredient ${ingredient_id}. Returning 202.`);
        return {
        ...
        console.error(`Error in triggerImage handler for ID ${ingredient_id}:`, error);
        return {
            statusCode: 500,
            body: JSON.stringify({
                error: {
